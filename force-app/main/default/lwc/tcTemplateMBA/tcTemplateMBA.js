...
import COLLEGE_LOGO from '@salesforce/resourceUrl/MBA_header';
...
export default class TcTemplateMBA extends LightningElement {
    ...
    async generatePDF() {
        ...
        for (let i = 0; i < this.studentData.length; i++) {
            ...
            const dateOfJoin = row.educx__Student_ID__r.educx__Joining_Date__c;
            ...
            const dateParts = dateOfJoin.split('-'); // Assuming date format is yyyy-mm-dd
            const year = dateParts[0];
            const month = dateParts[1];
            const day = dateParts[2];
            const doj = `${day}-${month}-${year}`; // Format as dd-mm-yyyy
            ...
            try {
                ...
                doc.setFontSize(20);
                doc.text('TRANSFER CERTIFICATE', 67, 67);

                doc.setFontSize(12);
                doc.setFont('times', 'italic');
                doc.text('S. No:', 15, imgHeight + 40);
                doc.setFont('times', 'bolditalic');
                doc.text(sno, 30, imgHeight + 40);
                doc.setFont('times', 'italic');
                doc.text('Date:', 150, imgHeight + 40);
                doc.setFont('times', 'bolditalic');
                doc.text(date, 165, imgHeight + 40);
                doc.setFont('times', 'italic');
                ...
                const fields = [
                    ...
                    { label: 'Date of Admission to the College', value: doj },
                ...
                fields.forEach((field, index) => {
                    ...
                    if (field.additional) {
                        ...
                        const additionalValueX = additionalLabelX + 25;
                        this.addText(doc, field.additional.value, additionalValueX, cursorY, fieldStyles.value);
                        this.addUnderline(doc, additionalValueX, cursorY + 1, 65);
                        ...
                        field.value.forEach((mole, moleIndex) => {
                            ...
                            this.addUnderline(doc, valueX, cursorY + 1, 170);
                        ...
                        this.addText(doc, field.value, valueX, cursorY, fieldStyles.value);
                        this.addUnderline(doc, valueX, cursorY + 1, isSpecialField ? 100 : 140); // Adjust underline width for special fields
