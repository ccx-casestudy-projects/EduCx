...
export default class BonafideTemplateMBA extends LightningElement {
    @api studentData;
    ...
    async generatePDF() {
        ...
        const todaydate = new Date();//format 2024-08-27T03:59:55.677Z
        const options = { day: 'numeric', month: 'long', year: 'numeric' };
        const currentDate = todaydate.toLocaleDateString('en-GB', options);//format 27 August 2024
        const today = todaydate.toISOString().split('T')[0];//format 2024-08-27 //toISOString() it converts todaydate to YYYY-MM-DDTHH:MM:SS.sssZ
        const time = todaydate.toTimeString().split(' ')[0];//format 03:59:55 //toTimeString() it converts todaydate to HH:MM:SS GMT±HHMM
        const imgWidth = 180;
        const imgHeight = 40;
        ...
        for (let i = 0; i < this.studentData.length; i++) {
            ...
            try {
                ...
                doc.setFontSize(20);
                doc.text('BONAFIDE CERTIFICATE', 67, 67);
